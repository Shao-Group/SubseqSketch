...
public:
    ...
    static void write_all(const Eigen::MatrixXi& embeds,
			  size_t num_embeds,
			  int embed_len,
			  int max_val,
    ...
    // and max_val(int)
    static Eigen::MatrixXi load_all(size_t& num_embeds,
				    int& embed_len,
				    int& max_val,
				    const std::string& embed_file);
    ...
    static void pairwise_cos_dist(const Eigen::MatrixXi& embed1,
				  const Eigen::MatrixXi& embed2_tran,
				  const std::string& dist_file);
